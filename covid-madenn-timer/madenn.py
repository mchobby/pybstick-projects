""" Programme du Minuteur Lave-Main - version MicroPython pour la PYBStick

	Inspiré du travail de madenn disponible sur http://madenn.space/minuteur-automatique-pour-lavage-mains

	Voir GitHub du projet pour plus d'information:

	https://github.com/mchobby/pybstick-projects/tree/master/covid-madenn-timer
...
def attente_savon():
	""" Attendre 4 secondes en éclairant l'anneau """
	global np # NeoPixels
	delay_ms = 4000 # 4 secondes
	...
	while (time.ticks_ms()-ref_ticks) < delay_ms:
		...
		try:
			iCount = int(NEOPIXEL_LEDS / (4000 / (time.ticks_ms()-ref_ticks)))
...
def lavage():
	...
	delay_ms = 40000 # 40 secondes
	ref_ticks = time.ticks_ms()
	while (time.ticks_ms()-ref_ticks) < delay_ms:
		# Number of LEDs to light up
		try:
			iCount = int(NEOPIXEL_LEDS / (40000 / (time.ticks_ms()-ref_ticks)))
		except ZeroDivisionError: # peut arriver sur une plateforme super rapide
			iCount=0
